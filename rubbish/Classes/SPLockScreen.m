...
//	CGFloat topOffset = radius;
	...
	for (int i=0; i < 9; i++) {
		...
		int column =  i % 3;
		int row    = i / 3;
		CGFloat x = 30+radius + column*(2*radius+36);//(gap + radius) + (gap + 2*radius)*column;//
		CGFloat y = (row * 36 + row * 2 * radius) + 60;
    ...
    CGFloat radius1 = 25.5;
    
    for (int i = 0; i < 9; i++) {
        NormalCircle *circle = [[NormalCircle alloc] initwithRadius:radius1];
        int column = i % 3;
        int row = i/3;
        CGFloat x1 = 25+ radius1 +column*(2*radius1 + 58);
        CGFloat y1 = (row * 65 + row*2*radius1) + 62-44;
        
        circle.center = CGPointMake(x1, y1);
		circle.tag = (row+kSeed)*kTagIdentifier + (column + kSeed);
		[self addSubview:circle];
...
- (NSNumber *) uniqueLineIdForLineJoiningPoint:(int)A AndPoint:(int)B
{
...
- (BOOL)hasVistedCircle:(int)circleId
{
    ...
    {
        if ((int)[number integerValue] == circleId)
        {
            hasVisit = YES;
            break;
        }
...
- (void)handlePanAtPoint:(CGPoint)point
{
	...
	if((cellPos >=0) && (cellPos != self.oldCellIndex))
        if (![self hasVistedCircle:(int)self.currentCellIndex]) {
	...
	else if(cellPos >= 0 && self.oldCellIndex != self.currentCellIndex)
	{
		...
		NSNumber *uniqueId = [self uniqueLineIdForLineJoiningPoint:(int)self.oldCellIndex AndPoint:(int)self.currentCellIndex];
...
- (void)gestured:(UIGestureRecognizer *)gesture
{
	...
	if([gesture isKindOfClass:[UIPanGestureRecognizer class]]){
		...
		if(cellPos >=0) {
			[self.cellsInOrder addObject:@(self.currentCellIndex)];
			[self performSelector:@selector(endPattern) withObject:nil afterDelay:0];
		}
	}
}
@end
