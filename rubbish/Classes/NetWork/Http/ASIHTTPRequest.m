...
- (void)clearDelegatesAndCancel
{
    ...
    for (ASIHTTPRequest *requet in [[Info getInstance] requestArray]) {
        if (requet.delegate == self.delegate&&![requet isEqual:self]) {
            [requet clearDelegatesAndCancel];
            
        }
        else if (requet.delegate == self.delegate && [requet.postBody isEqual:self.postBody]) {
            [deleteArray addObject:requet];
//            [[[Info getInstance] requestArray] removeObject:requet];
        }
    
    }
    if ([deleteArray count]) {
        [[[Info getInstance] requestArray] removeObjectsInArray:deleteArray];
    }
...
- (void)loginViewControllerFunc{
    ...
    NSArray * views = nvc.viewControllers;
    UIViewController * viewController = [views lastObject];
...
- (void)startAsynchronous
{
    ...
    [[self requestHeaders] setValue:[UDIDFromMac uniqueGlobalDeviceIdentifier] forKey:@"macCode"];
    [[self requestHeaders] setValue:[NSString stringWithFormat:@"%@_%@",Name,[[Info getInstance] cbVersion]] forKey:@"versinoNum"];
    [[self requestHeaders] setValue:newVersionKey forKey:@"newVersion"];
    [[self requestHeaders] setValue:[[Info getInstance] cbVersion] forKey:@"version"];
    [[self requestHeaders] setValue:[[Info getInstance] cbSID] forKey:@"sid"];
    if ([[[Info getInstance] userId] length]) {
        [[self requestHeaders] setValue:[[Info getInstance] userId] forKey:@"userId"];
    }
...
- (void)reportFinished
{
    ...
    if( responseStatusCode != 200  )
    {
        ...
        if(error == Nil)
        {
             error = [[NSError errorWithDomain:@"服务端错误" code:ASICompressionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"错误代码 %d",responseStatusCode],NSLocalizedDescriptionKey,nil]] retain];
        }
        [self reportFailure];
        return ;
    }
    ...
    if ([[response JSONValue] isKindOfClass:[NSDictionary class]]) {
        NSDictionary * dict = [response JSONValue];
        if ([dict objectForKey:@"security_code"]) {
            if ([[dict objectForKey:@"security_code"] intValue] != 1) {
                NSLog(@"security_code = %ld", (long)[[dict objectForKey:@"security_code"] integerValue]);
                NSLog(@"url = %@", self.url);
                [[NSUserDefaults standardUserDefaults] setValue:@"" forKey:@"logincp"];
                caiboAppDelegate *cai = [caiboAppDelegate getAppDelegate];
                [cai showMessage:@"基于安全因素,请重新登录"];
                [self outCaiPiaoFunc];
                [self loginViewControllerFunc];
...
- (void)reportFailure
{
    if (![[self error].localizedDescription isEqualToString:@"The request was cancelled"]) {
        [MobClick event:@"event_netRrror2" label:[NSString stringWithFormat:@"%ld_%@",(long)[self error].code ,[self error].localizedDescription]];
    }
	if (delegate && [delegate respondsToSelector:didFailSelector]) {
		[delegate performSelector:didFailSelector withObject:self];
    }else if (responseStatusCode >= 400 && !dontShowAlert) {
//        if (![[[UIApplication sharedApplication].windows objectAtIndex:0] viewWithTag:112345678]) {
//            CP_UIAlertView * failAlertView = [[CP_UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"%d",responseStatusCode] delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定",nil];
