...
#define LOTTERY_ID @"014"

@interface CQShiShiCaiViewController ()

@end

@implementation CQShiShiCaiViewController
...
- (id)init {
    self = [super init];
    if (self) {
        wanArray = [[NSArray alloc] initWithObjects:@"大小单双",@"一星直选",@"二星直选",@"三星直选",@"五星直选",@"五星通选",nil];
        modetype = SSCwuxingfushi;
        lotterytype = TYPE_CQShiShi;
        NSInteger type = [[[NSUserDefaults standardUserDefaults] valueForKey:@"CQShiShiCaiDefaultModetype"] integerValue];
...
-(ModeTYPE)changeStringToMode:(NSString *)str {
    ...
    else if ([str isEqualToString:@"一星直选"]){
        return SSCyixingfushi;
    }
    else if ([str isEqualToString:@"二星直选"]){
...
- (void)LoadIphoneView {
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(50+30+10, 0, 270, 44)];
    titleView.backgroundColor = [UIColor clearColor];
    titleView.userInteractionEnabled = YES;
...
//-----------------------------------------------bianpinghua by sichuanlin
	yaoImage.image = UIImageGetImageFromName(@"YIY960.png");
//    yaoImage.frame =CGRectMake(10, 40, 15, 15);
//    yaoImage.image = UIImageGetImageFromName(@"yaoyiyao.png");
...
//    sqkaijiang = [[ColorView alloc] initWithFrame:CGRectMake(150, 20, 150, 23)];
    sqkaijiang = [[ColorView alloc] initWithFrame:CGRectMake(15, 11, 150, 15)];
...
//-------------------好像可以
        ...
        //        {
        //            NSLog(@"%d",desLabel.tag);
        //            if(i%2==0)
        //                desLabel.text=@"至少选两个号码";
        //        }
        if (i == 0 || i == 1)
        {
...
//        }
//        [clearBtn addTarget:self action:@selector(clearAllTap:) forControlEvents:UIControlEventTouchUpInside];
//        clearBtn.tag=2050+i;
//        [firstView addSubview:clearBtn];
//        
//        UIImageView *lajitonggaiIma=[[UIImageView alloc]init];
//        if (SWITCH_ON)
//        {
...
//        }
//        lajitonggaiIma.image=[UIImage imageNamed:@"lajitonggai.png"];
//        lajitonggaiIma.hidden=YES;
//        lajitonggaiIma.tag=4321;
//        [firstView addSubview:lajitonggaiIma];
//        [lajitonggaiIma release];
//        
//        UIImageView *lajitongIma=[[UIImageView alloc]init];
//        if (SWITCH_ON)
//        {
...
//        }
//        lajitongIma.tag=2060+i;
//        lajitongIma.hidden=YES;
//        lajitongIma.image=[UIImage imageNamed:@"lajitong.png"];
//        [firstView addSubview:lajitongIma];
//        [lajitongIma release];
...
//            label.jianjuHeight = 3;
            ...
            firstView.backgroundColor = [UIColor clearColor];
//            if (SWITCH_ON) {
//                firstView.frame = CGRectMake(10, 47 + i*123, 300, 118);
//            }else{
//                firstView.frame = CGRectMake(10, 73 + i*90, 300, 91);
//            }
            if (SWITCH_ON) {
            ...
            if (i < 2) {
                //                if (SWITCH_ON) {
                //                    firstView.frame = CGRectMake(10, 47 + i*123, 300, 90);
                //                }else{
//                firstView.frame = CGRectMake(10, 47 + i*123, 300, 80);
                //                }
...
//                if (SWITCH_ON)
//                {
//                    clearBtn.frame=CGRectMake(4*70, 35+8, 30, 28);
//                    lajitonggaiIma.frame=CGRectMake(4*70+3, 35+8, 18, 5);
//                    lajitongIma.frame=CGRectMake(4*70+3, 35+14, 18, 18);
//                }
//                else
//                {
//                    clearBtn.frame=CGRectMake(4*70, 35+8, 30, 28);
//                    lajitonggaiIma.frame=CGRectMake(4*70+3, 35+8, 18, 5);
//                    lajitongIma.frame=CGRectMake(4*70+14+3, 35+14, 18, 18);
//                }
            }
            else if (i>6){
                if(i == 7)
...
//        UIView *clearBtn=[v viewWithTag:2050+i-1000];
//        UIView *lajitongIma=[v viewWithTag:2060+i-1000];
        GifButton * trashButton = (GifButton *)[v viewWithTag:333];
        GCBallView * lastBallView;
        if (i >= 1002) {
            if (SWITCH_ON) {
//                v.frame = CGRectMake(10, heght, 300, 118);
                v.frame = CGRectMake(0, 85 + i*150, 320, 150);
//                clearBtn.frame=CGRectMake(4*51+14+3, 52+35+4, 30, 28);
//                lajitongIma.frame=CGRectMake(4*51+14+9, 52+35+4+2+6, 18, 18);
                //                [v viewWithTag:99].hidden = NO;
            }else{
//                v.frame = CGRectMake(10, heght, 300, 91);
...
//            int a= j/5,b = j%5;
            ...
            GCBallView * ballView = (GCBallView *)[v viewWithTag:j];
//            if (SWITCH_ON) {
//                ballView.frame = CGRectMake(b*50+55, a*52+7, 35, 35);
//                ballView.ylLable.hidden = NO;
//            }else{
//                ballView.frame = CGRectMake(b*50+55, a*37+7, 35, 35);
//                ballView.ylLable.hidden = YES;
//            }
            if (SWITCH_ON) {
...
- (void)timeChange {
    ...
    if (seconds >=0) {
        mytimeLabel3.text = [NSString stringWithFormat:@"%02d:%02d",(int)(seconds/60),seconds%60];
        if(seconds/60 == 0)
        {
    ...
    if (!isKaiJiang) {
        ...
        if ((seconds <= 600 - yanchiTime-180 && seconds % 5 == 0)) {
            canRefreshYiLou = NO;
            [self getIssueInfo];
        }
    }
    ...
    if(titleBtnIsCanPress)
    {
        ...
        if(!isShowing)
        {
            ...
            for (CP_XZButton *btn in alert2.backScrollView.subviews) {
                if ([btn isKindOfClass:[CP_XZButton class]] &&btn.tag == type) {
                    btn.selected = YES;
...
//        titleLabel.text = [NSString stringWithFormat:@"%@-%@期",[self changeModetypeToString:modetype],self.issue];
    ...
    switch (modetype) {
        case SSCdaxiaodanshuang:
        {
            ...
            if ([[[Info getInstance] userId] intValue] == 0) {
                tl.frame = CGRectMake(0, 0, 220, 44);
//                la.frame = CGRectMake(175, 14, 17, 17);
                la.frame = CGRectMake(125, 14, 17, 17);
            }else {
                
                tl.frame = CGRectMake(12, 0, 220, 44);
//                la.frame = CGRectMake(191, 14.5, 17, 17);
                la.frame = CGRectMake(140, 14.5, 17, 17);
            }
            
            colorLabel.text = @"猜中开奖号最后2位的属性即中<4>元";
            infoLable.text = @"每位至少选择1个数字";
...
//                la.frame = CGRectMake(175, 14, 17, 17);
                ...
                tl.frame = CGRectMake(15, 0, 220, 44);
//                la.frame = CGRectMake(191, 14.5, 17, 17);
                la.frame = CGRectMake(140, 14.5, 17, 17);
            }
...
//                la.frame = CGRectMake(191, 14.5, 17, 17);
        ...
        case SSCrenxuanyi:
        {
            ...
            UILabel *la = (UILabel *)[self.CP_navigation.titleView viewWithTag:567];
            if ([[[Info getInstance] userId] intValue] == 0) {
                tl.frame = CGRectMake(27, 0, 220, 44);
...
- (void)pressBgButton:(UIButton *)sender{
    if (sender.tag <= [wanArray count]) {
        ...
        [[NSUserDefaults standardUserDefaults] setValue:[NSString stringWithFormat:@"%d",modetype] forKey:@"CQShiShiCaiDefaultModetype"];
    }
...
- (void)ballSelectChange:(UIButton *)imageView {
	...
	NSLog(@"%@,%lu",[self seletNumber],(unsigned long)bets);
