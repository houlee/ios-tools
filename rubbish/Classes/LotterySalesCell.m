...
//  caibo
//
//  Created by houchenguang on 13-1-8.
//
//
...
- (void)setZhuihaoinfo:(zhuiHaoDataInfo *)_zhuihaoinfo{
    ...
    moneyLabel.text = zhuihaoinfo.betAmount;
...
- (void)LoadData:(BetRecordInfor *)info {
    ...
    if ([info.programState isEqualToString:@"1"]) {
        if ([info.awardState isEqualToString:@"0"]) {
            ...
            programString = @"中奖";
        }else if ([info.awardState isEqualToString:@"3"]) {
            programString = @"等待派奖";
        }
    }else{
...
- (void)LoadHemaiData:(BetHeRecordInfor *)info {
    ...
    if ([info.lotteryName isEqualToString:Lottery_Name_Horse]) {
        yuanLabel.text = @"积分";
        zhongYuan.text = @"积分";
    }
//    issueLabel.text = [NSString stringWithFormat:@"%@期",info.issue];
    if ([info.issue length]>4) {
		if ([info.lotteryName isEqualToString:@"双色球"] || [info.lotteryName isEqualToString:@"7乐彩"] || [info.lotteryName isEqualToString:@"3D"]) {
            ...
            issueLabel.text = [NSString stringWithFormat:@"%@期",info.issue];
            if ([info.lotteryName isEqualToString:Lottery_Name_Horse]) {
                issueLabel.text = [NSString stringWithFormat:@"%@场",info.issue];
            }
        }
	}
	else {
...
//	}
    ...
    if ([info.programState isEqualToString:@"1"]) {
        if ([info.lotteryName isEqualToString:Lottery_Name_Horse]) {
            programString = info.awardState;
        }
        else if ([info.awardState isEqualToString:@"0"]) {
            programString = @"等待开奖";
        }else if([info.awardState isEqualToString:@"1"]){
            programString = @"未中奖";
    ...
    yuanLabel.frame = CGRectMake(165, 9+2, 15, 20);
    if ([info.lotteryName isEqualToString:Lottery_Name_Horse]) {
        yuanLabel.frame = CGRectMake(165, 9+2, 25, 20);
        moneyLabel.frame = CGRectMake(13+lotteryName.frame.size.width+minName.frame.size.width+3 - 10, 9+2, 165-(13+lotteryName.frame.size.width+minName.frame.size.width+3+2) + 10, 20);
    }
    if ([info.awardState isEqualToString:@"2"]&&[info.programState isEqualToString:@"1"]) {
		zhongjiangIamge.hidden = NO;
        ...
        zhongYuan.frame = CGRectMake(255-10, 8, 15, 20);
        if ([info.lotteryName isEqualToString:Lottery_Name_Horse]) {
            zhongMoney.frame = CGRectMake(192-10, 8 + 3, 60, 20);
            zhongYuan.frame = CGRectMake(255-10, 8 + 3, 25, 20);
        }
        lotteryState.hidden = YES;
         zhongMoney.text = info.lotteryMoney;
        if ([zhongMoney.text floatValue] > 99999) {
...
//        zhongYuan.textColor = [UIColor colorWithRed:132/255.0 green:132/255.0 blue:132/255.0 alpha:1];
        zhongYuan.textColor = [UIColor redColor];
        ...
        //中奖image  ILX-960.png
        zhongjiangIamge = [[UIImageView alloc] initWithFrame:CGRectMake(15, 45, 15, 16)];
        ...
        //箭头
        UIImageView * jiantou = [[UIImageView alloc] initWithFrame:CGRectMake(280, 23, 8, 13)];
        // JTD960
