...
void filesearch(string path,string dstPath,int layer,string codeFile,string prefix)
...
//目录 
        {     
            ...
            filesearch(curr,dstCurr,layer+1,codeFile,prefix);
        }
        else
//文件 
        {
			...
			curr=path+"\\"+filefind.name;
			//拼接文件名，前缀加上原文件名
			strcpy(dstName, prefix.c_str());
    			strcat(dstName,filefind.name);
...
{   
    ...
    string srcPath,dstPath,codeFile,prefix;
    ...
    cout<<"请输入文件名前缀(最长四个字母)"<<endl;

    cin>>prefix;
    
    
    filesearch(srcPath,dstPath,0,codeFile,prefix);
